...
Improvement #4: Now injects 6 anomaly scenarios across multiple channels
to ensure demo reliability. All channels will have detectable anomalies.
"""
import pandas as pd
import numpy as np
from datetime import datetime, timedelta
...
def generate_channel_data(
    ...
    for i, date in enumerate(DATES):
        ...
        # Apply conversion anomaly (e.g., Pixel Death)
        conversions = int(conversions * conv_multiplier)
...
# ============================================================================
# 1. DEFINING THE SCENARIOS (Improvement #4: 6 scenarios across 6 channels)
# ============================================================================

ANOMALIES = [
    {
        # SCENARIO 1: The "Obvious" Spike (Google PMax)
        # PMax goes crazy spending budget on low-quality YouTube placements
        "channel": "google_pmax",
        "start_day": 3, "end_day": 0,  
        "spend_mult": 3.0,  # CRITICAL: 3x Spend
        "roas_mult": 0.3,   # CRITICAL: 70% Drop in ROAS
    ...
    {
        # SCENARIO 2: The "Silent Killer" (Meta Ads Pixel Death)
        # Spend is normal, but conversions drop to near zero
        "channel": "meta_ads",
        "start_day": 2, "end_day": 0,
        "spend_mult": 1.0,
        "conv_mult": 0.05,  # CRITICAL: 95% Drop in Conversions
    ...
    {
        # SCENARIO 3: LinkedIn CPL Creep (NEW)
        # Gradual cost increase over 5 days
        "channel": "linkedin_ads",
        "start_day": 5, "end_day": 0,
        "cpa_mult": 2.5,    # HIGH: CPA doubles over 5 days
        "spend_mult": 1.3,
        "description": "LinkedIn CPL Creep"
    },
    {
        # SCENARIO 4: TV Reach Collapse (NEW)
        # Sudden drop in impressions (preemptions or measurement gap)
        "channel": "tv",
        "start_day": 2, "end_day": 0,
        "spend_mult": 1.0,  # Spend unchanged
        "conv_mult": 0.3,   # 70% fewer conversions
        "roas_mult": 0.35,  # ROAS tanks
        "description": "TV Reach Drop (preemptions)"
    },
    {
        # SCENARIO 5: Affiliate Conversion Cliff (NEW)
        # Top affiliate removed tracking link
        "channel": "affiliate",
        "start_day": 3, "end_day": 0,
        "conv_mult": 0.15,  # 85% conversion drop
        "roas_mult": 0.2,
        "description": "Affiliate Link Removed"
    },
    {
        # SCENARIO 6: Podcast Engagement Dip (NEW)
        # Holiday weekend + truncated host read
        "channel": "podcast",
        "start_day": 4, "end_day": 0,
        "conv_mult": 0.4,   # 60% drop in conversions
        "cpa_mult": 1.8,
        "description": "Podcast Low Engagement"
    },
]
...
def generate_influencer_data() -> pd.DataFrame:
    """Generate mock influencer campaign data with a fraud anomaly."""
    creators = {
        "INF_001": {"name": "TechGuru", "platform": "youtube", "base_engagement": 0.04, "spend": 5000, "base_impressions": 200000},
        "INF_002": {"name": "FitLife", "platform": "instagram", "base_engagement": 0.05, "spend": 6000, "base_impressions": 250000},
        "INF_003": {"name": "MomHacks", "platform": "instagram", "base_engagement": 0.06, "spend": 3500, "base_impressions": 120000},
        "INF_004": {"name": "GamerX", "platform": "twitch", "base_engagement": 0.055, "spend": 5000, "base_impressions": 180000},
        "INF_005": {"name": "StyleIcon", "platform": "instagram", "base_engagement": 0.045, "spend": 7000, "base_impressions": 300000},
        # The "Bad Actor" - Obvious fraud signals
        "INF_999": {"name": "ViralViper", "platform": "tiktok", "base_engagement": 0.001, "spend": 15000, "base_impressions": 500000},
    }
    
    data = []
    for date in pd.date_range(start=END_DATE - timedelta(days=30), end=END_DATE, freq="3D"):
        for creator_id, info in creators.items():
            impressions = int(add_noise(info["base_impressions"], 0.3))
            engagement_rate = add_noise(info["base_engagement"], 0.15)
            engagements = int(impressions * engagement_rate)
            clicks = int(engagements * add_noise(0.2, 0.1))
            conversions = int(clicks * add_noise(0.05, 0.2)) if creator_id != "INF_999" else 0
...
# ============================================================================
# 3. RAG HISTORY (40+ incidents with expanded channels)
# ============================================================================
...
def generate_rag_history():
    templates = [
        ...
        {
            "type": "Response Rate Drop",
            "channel": "direct_mail",
            "roots": [
                "Mailing list had high percentage of outdated addresses.",
                "Postal delays due to holiday volume caused late delivery.",
                "Creative execution (envelope design) performed poorly in testing.",
            ],
            "fixes": [
                "Ran NCOA update on mailing list before next campaign.",
                "Shifted future mailings to avoid peak postal periods.",
                "A/B tested new envelope designs with stronger hook.",
            ]
        },
        {
            ...
            "roots": [
                ...
                "Geofencing radius was set too small for attribution.",
            ],
            "fixes": [
